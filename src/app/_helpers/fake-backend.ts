...
const user = localStorage.getItem(accountsKey);
let accounts = user ? JSON.parse(user) : [];
...
export class FakeBackendInterceptor implements HttpInterceptor {
  intercept(
      ...
      return from(
        ...
        concatMap((data: any) => {
          ...
          let account = accounts?.find(
            (x: { facebookId: any }) => x.facebookId === data.id
          );
          if (!account) {
    ...
    function unauthorized(message = 'Unauthorized') {
      return throwError({ status: 401, error: { message } }).pipe(
        materialize(),
        delay(500),
        dematerialize()
      );
    ...
    function newAccountId() {
      return accounts.length
        ? Math.max(...accounts.map((x: { id: any }) => x.id)) + 1
        : 1;
    }
